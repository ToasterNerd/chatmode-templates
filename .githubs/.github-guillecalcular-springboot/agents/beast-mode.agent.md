...
handoffs:
  - label: Update Instructions
    ...
    prompt: Analyze the recent code changes and update project instructions if needed.
    send: false
  - label: Plan Feature
    agent: Plan
...
**You MUST automatically invoke the `instructions-curator` subagent using `runSubagent` tool when ANY of these conditions are met:**

1. **New patterns established**: You've created a new code pattern that will be used repeatedly (3+ times)
2. **New conventions**: You've established a new naming convention
3. **New components**: You've created reusable components/services/utilities
4. **Stack changes**: You've added or updated important dependencies
5. **New workflows**: You've established new development/testing processes
...
5. ✅ Inform user: "Tarea completada" (+ mention if instructions were updated)
...
```
prompt: "Analyze changes made in AFPGeneralService.java where I implemented 
a new caching pattern with @Cacheable. Determine if this pattern should be documented in 
java-backend.instructions.md as standard for legal data services."

description: "Update instructions for caching pattern"
```

### ⚠️ IMPORTANT

- Subagent only updates `.instructions.md` and `copilot-instructions.md` files
- Updates must be incremental and non-destructive
- Always maintain existing file format and style
- Document in Spanish (project requirement)
